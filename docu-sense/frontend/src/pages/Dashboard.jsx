...
export default function Dashboard() {
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      {/* Main Content */}
      <main className="flex-1 p-6 md:p-10 mt-12">
        {/* Header Section */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-10 gap-5">
          <div>
            <h1 className="text-3xl font-extrabold text-gray-900 mb-2 tracking-wide">
              Welcome back <span className="text-blue-600"></span> 👋
            </h1>
            <p className="text-sm text-gray-500 flex items-center gap-2 font-medium">
              <Clock size={16} />
              Last login: {new Date().toLocaleDateString("en-US", { weekday: 'long', month: 'short', day: 'numeric' })} at {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
            </p>
          </div>
          <div className="relative w-full sm:w-96">
            ...
            <Input
              type="text"
              placeholder="Search the documents"
              className="pl-12 w-full rounded-full border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-md transition-shadow duration-300 text-gray-800"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-10">
          <Link to="/upload-document">
            <Button className="h-24 bg-white hover:bg-blue-50 border-2 border-dashed border-blue-300 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all shadow-md hover:shadow-lg cursor-pointer w-full">
              <Upload className="text-blue-600" size={26} />
              <span className="text-sm font-semibold text-blue-700">Upload New Document</span>
            </Button>
          </Link>

          <Link to="/all-document">
            <Button className="h-24 bg-white hover:bg-purple-50 border-2 border-dashed border-purple-300 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all shadow-md hover:shadow-lg cursor-pointer w-full">
              <Zap className="text-purple-500" size={26} />
              <span className="text-sm font-semibold text-purple-700">All Document</span>
            </Button>
          </Link>

          <Button className="h-24 bg-white hover:bg-green-50 border-2 border-dashed border-green-300 rounded-2xl flex flex-col items-center justify-center gap-3 transition-all shadow-md hover:shadow-lg cursor-pointer">
            <Share2 className="text-green-600" size={26} />
            <span className="text-sm font-semibold text-green-700">Smart Analysis</span>
          </Button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {[
            ...
            <Card
              key={idx}
              className={`bg-white rounded-2xl border border-gray-200 hover:shadow-2xl transition-shadow duration-300`}
            >
              <CardContent className="p-6">
                ...
                <div className="flex items-end justify-between">
                  ...
                  <div className="relative w-1/2 h-3 rounded-full bg-gray-200 overflow-hidden shadow-inner">
                    <div
                      className={`h-3 bg-${stat.color}-600 rounded-full transition-all duration-700 ease-in-out`}
                      style={{ width: `${stat.progress}%` }}
                    ></div>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white rounded-2xl p-8 shadow-md border border-gray-200">
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-3 text-blue-700">
              <Upload size={22} />
              Recent Uploads
            </h2>
            <div className="space-y-5">
              {[
                ...
                <div
                  key={idx}
                  className="flex items-center gap-5 p-4 rounded-xl hover:bg-blue-50 transition-colors cursor-pointer shadow-sm"
                >
                  <div className="bg-blue-100 p-3 rounded-lg">
                    <FileText className="text-blue-600" size={20} />
                  </div>
                  <div className="flex-1">
                    <h3 className="font-semibold text-gray-900 truncate">{upload.name}</h3>
                    <p className="text-sm text-gray-500">{upload.type} • {upload.size}</p>
                  </div>
                  <span className="text-sm text-gray-500">{upload.date}</span>
                </div>
          ...
          {/* System Activity */}
          <div className="bg-white rounded-2xl p-8 shadow-md border border-gray-200">
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-3 text-purple-700">
              <Zap size={22} />
              System Activity
            </h2>
            <div className="space-y-5">
              {[
                ...
                <div
                  key={idx}
                  className="flex items-center gap-4 p-4 rounded-xl hover:bg-purple-50 transition-colors cursor-pointer shadow-sm"
                >
                  <div
